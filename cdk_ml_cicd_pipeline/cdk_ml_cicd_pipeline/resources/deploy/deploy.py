...
class Deploy(Construct):
    ...
    def __init__(self, scope: Construct, stack_name: str, component_id: str, **kwargs):
        ...
        CfnOutput(self,
        id="SlackCommandAPIGatewayEndpoint",
        export_name="SlackCommandAPIGatewayEndpoint",
        value=self._http_api_slack_command.api_endpoint)
    ...
    def create_slack_command(self) -> Resource:
        """Slack Command用Lambda

        Returns:
            Resource: lambda
        """

        lambdaFn_name = f"{self.stack_name}_slack_command"
        role_name = f"{self.stack_name}_slackcommands_role" 
        ...
        lambda_role.attach_inline_policy(
            aws_iam.Policy(self, "AllowSlackCommand",
                document=aws_iam.PolicyDocument(
                    statements=[
                        aws_iam.PolicyStatement(
                            actions=[
                            ...
                            resources=[self._stepfunction.state_machine_arn]
                        )
                    ]
                )
            )
        ...
        lambdaFn_path = str(pathlib.Path(__file__).resolve().parent) + "/slack_command/"
        print(self._stepfunction.state_machine_arn)
        lambdaFn = aws_lambda.Function(
            self,
            id=lambdaFn_name,
            function_name=lambdaFn_name,
            ...
            timeout=Duration.seconds(10),
            runtime=aws_lambda.Runtime.PYTHON_3_9,
            description="SlackCommand用Lambda作成",
            role=lambda_role,
            environment={
                "STATEMACHINEARN": self._stepfunction.state_machine_arn
            }
        )

        slack_lambda_integration = HttpLambdaIntegration("SlackLambdaIntegration", lambdaFn)
        ...
        http_api.add_routes(
            ...
            integration=slack_lambda_integration
        )

        return lambdaFn,http_api
    ...
    def create_deploy_pipeline(self) -> Resource:
        ...
        self._lambda_slack_command,self._http_api_slack_command = self.create_slack_command()